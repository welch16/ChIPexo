...
{
  fwd_strand = t(do.call(cbind,mclapply(as.character(1:length(regions)),function(i,reads)fwd_strand_ratio(reads[[i]]),reads,mc.cores = mc)))
  dif_pos = t(do.call(cbind,mclapply(as.character(1:length(regions)),function(i)diff_pos_summit(reads[[i]]),mc.cores = mc)))
...
save(file = file.path(datadir,paste0("output",regionsFile)),list = c("regions","exo1","exo2","pet1","pet2","set1","set2"))

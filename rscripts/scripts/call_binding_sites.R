...
if("--help" %in% args){
  ...
  Arguments:
   ...
   -- peaksdir
      ...
      The directrory where the peaks are stored. The code is gonna match it with readsfile.
...
peaksfiles <- list.files(peaksdir)
peaksfiles <- peaksfiles[grep(idx[1],peaksfiles)]
peaksfiles <- peaksfiles[grep(idx[3],peaksfiles,fixed = TRUE)]
peaksfile <- file.path(peaksdir,peaksfiles)

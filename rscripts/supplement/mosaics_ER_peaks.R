...
## ER - exo
er_files <- files[grep("ER",files)]


fit_ER <- list()
fit_ER[["exo1"]] <- fit_mosaics_OS(er_files[4],
   addfiles,bgEst = "matchLow")
fit_ER[["exo2"]] <- fit_mosaics_OS(er_files[5],
   addfiles,bgEst = "matchLow")
fit_ER[["exo3"]] <- fit_mosaics_OS(er_files[6],
   addfiles,bgEst = "matchLow")
fit_ER[["seq1"]] <- fit_mosaics_IO(er_files[1],input[4],bgEst = "matchLow")
fit_ER[["seq2"]] <- fit_mosaics_IO(er_files[2],input[4],bgEst = "matchLow")
fit_ER[["seq3"]] <- fit_mosaics_IO(er_files[3],input[4],bgEst = "matchLow")



pdf(file = "figs/ER_gof.pdf")
u <- lapply(fit_ER,plot)
dev.off()
...
peaks <- lapply(fit_ER,call_peaks,thres , fdr)

save(peaks,file = "data/ER_peaks.RData")

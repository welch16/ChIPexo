...
chip_dirs <- list()
chip_dirs[["exo"]] <- "/p/keles/ChIPexo/volume4/landick_data/ChIPexo"
chip_dirs[["pet"]] <- "/p/keles/ChIPexo/volume4/landick_data/ChIPseq_PET"
chip_dirs[["set"]] <- "/p/keles/ChIPexo/volume4/landick_data/ChIPseq_SET"
...
save(reads,file = "data/figures/fig1/fig1B_bins.RData")
...
exo = rbind((reads[[1]] %>% readsF)$U00096,(reads[[1]] %>% readsR)$U00096) %>% dt2gr
exo = resize(exo,150)

pet = rbind((reads[[2]] %>% readsF)$U00096,(reads[[2]] %>% readsR)$U00096) %>% dt2gr

set = rbind((reads[[3]] %>% readsF)$U00096,(reads[[3]] %>% readsR)$U00096) %>% dt2gr
set = resize(set,150)


mcols(bins)$ChIPexo = countOverlaps(bins,exo)
mcols(bins)$ChIPseqPET = countOverlaps(bins,pet)
mcols(bins)$ChIPseqSET = countOverlaps(bins,set)

library(readr)

write_tsv(bins %>% as.data.frame %>% as.tbl,path = "data/figures/fig1/fig1B_bins.tsv")
...
write_tsv(strand_count,path = "data/figures/fig1/fig1E_FSR.tsv")

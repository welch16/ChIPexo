...
ggsave(file.path(opt$outdr,"fig2B.png"),region_comp)

## FSR dist

FSRdist = files[grep("FSR",files)] %>% read_tsv %>% filter(Repl == "Rep-1")
...
ggsave(file.path(opt$outdr,"fig2C.png"),FSR)
...
ggsave(file.path(opt$outdr,"fig2D.png"),width = 9,betascore)


all = arrangeGrob(arrangeGrob(ARC,betascore,nrow = 1,ncol = 2),

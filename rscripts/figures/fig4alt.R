...
ggsave(file.path(opt$outdr,"fig4Aalt.png"),foxA1_boxplot)

## Profiles

prof = files[grepl("profiles_full.t",files)] %>% read_tsv
                                
profiles = prof %>% filter(strand == "+") %>%
    group_by(coord,replicate,Set) %>% summarize(signal = mean(counts)) %>% ungroup %>%
    mutate(replicate = as.character(replicate),
           replicate = ifelse(replicate == "Rep1","Rep-1",
                              ifelse(replicate == "Rep2","Rep-2","Rep-3")))
...
profiles_rev = prof %>% filter(strand == "-") %>%
    group_by(coord,replicate,Set) %>% summarize(signal = mean(counts)) %>% ungroup %>%
    mutate(replicate = as.character(replicate),
           replicate = ifelse(replicate == "Rep1","Rep-1",
                              ifelse(replicate == "Rep2","Rep-2","Rep-3")))
...
prof2 = files[grepl("profiles_full_pe",files)] %>% read_tsv


profiles2 = prof2 %>% filter(strand == "+") %>%
    group_by(coord,replicate,Set) %>% summarize(signal = mean(counts)) %>% ungroup %>%
    mutate(replicate = as.character(replicate),
           replicate = ifelse(replicate == "Rep1","Rep-1",
                              ifelse(replicate == "Rep2","Rep-2","Rep-3")))
...
ggsave(file.path(opt$outdr,"fig4B_rev.png"),profiles_rev)

ggsave(file.path(opt$outdr,"fig4Balt.png"),profiles)

ggsave(file.path(opt$outdr,"Supplement_possible.png"),profiles2)
...
ggsave(file.path(opt$outdr,"fig4Calt.png"),FSR_hist)    
...
ggsave(file.path(opt$outdr,"fig4Dalt.png"),TBP_boxplot)
...
ggsave(file.path(opt$outdr,"fig4Ealt.png"),TBP_FSR,width = 12)    
...
ggsave(file = file.path(opt$outdr,"fig4alt.pdf"),all,
       width = 180,
       height = 280,
       units = "mm")

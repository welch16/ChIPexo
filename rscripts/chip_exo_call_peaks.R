...
bins <- mclapply(file.path(bin_dir,files),read_bins_wrap,map_file,GC_file,N_file,opt,mc.cores = mc)

## exploratory plots

pdf(file = file.path(figs_dir,"chip_exo_bins.pdf"))
u <- lapply(bins,plot)
dev.off()
...
pdf(file = file.path(figs_dir,"chip_exo_gof.pdf"))
u <- lapply(fits,plot)
dev.off()
...
out_dir <- "/p/keles/ChIPexo/volume6/results/mosaics_peaks/Landick/ChIPexo/peaks"

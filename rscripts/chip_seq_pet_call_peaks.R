...
pdf(file = file.path(figs_dir,"chip_seq_pet_bins.pdf"))
u <- lapply(bins,plot)
dev.off()
...
pdf(file = file.path(figs_dir,"chip_seq_pet_gof.pdf"))
u <- lapply(fits,plot)
dev.off()
...
out_dir <- "/p/keles/ChIPexo/volume6/results/mosaics_peaks/Landick/ChIPseq_PET/peaks"

## out_files <- gsub(".filter.bam_fragL150_bin150.txt","_peaks.txt",files)
out_files <- gsub(".filter.bam_bin150.txt","_peaks.txt",files)[-1]

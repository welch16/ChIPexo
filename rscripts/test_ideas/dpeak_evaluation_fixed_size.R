...
dr <- "/p/keles/ChIPexo/volume7/Landick/K12/sampled"
...
## save(fits, file = "data/dpeak_experiment_fits_same_sample.RData")
load(file = "data/dpeak_experiment_fits_same_sample.RData")
...
pdf(file = "figs/resolution_experiment_fixed_regions_same_depth.pdf")
ggplot(resoDT,aes(seq , reso,colour =seq))+geom_boxplot()+coord_cartesian(ylim = c(0,150))+
  facet_grid(rif ~ rep)+theme_bw()+theme(legend.position = "none")+scale_color_brewer(palette = "Set1")
dev.off()
...
sens_plot <- function(sens,rif,repl,minreads,peaks)
{
  idx <- which(peaks[[1]] > minreads)
  sens1 <- sens[[1]][idx]
  sens2 <- sens[[2]][idx]
  ...
  sens3 <- data.table(do.call(rbind,lapply(sens3,unlist)))
  sens <- rbind(sens1[,seq := "ChIPexo"],sens2[,seq:="ChIPseq_PET"],sens3[,seq:= "ChIPseq_SET"])
  sens[,rif := rif]
  sens[,repl := repl]
...
pdf(file = "figs/profiles/resolution_sensitivity_experiment_fixed_depth.pdf")
sens_plot(sens,"20min","rep-2",minreads,peaks)
dev.off()
...
## pdf(file = "figs/profiles/EColi_methods_comp_sampled.pdf",width = 9,height = 12)

...
figsdir = "figs/NAR_review/Threshold_analysis/TBPnexus"
...
pdf(paste0(figsdir,"_ARCvURC_threshold.pdf"),height = 4,width = 6)
...
## print(ARCvURCplot(exoList[[3]]) + ggtitle("rep3") + xlim(0,5))
dev.off()


pdf(paste0(figsdir,"_FSRdist_threshold.pdf"),height = 7,width = 6)
...
## print(FSRDistplot(exoList[[3]],depth.values = seq_len(120),quantiles = c(.1,.25,.5,.75,.9)) + ggtitle("rep3"))
dev.off()


pdf(paste0(figsdir,"_RegionComp_threshold.pdf"),height = 7,width = 6)
...
## print(regionCompplot(exoList[[3]],depth.values = seq_len(50)) + ggtitle("rep3"))
dev.off()
...
## print(paramDistBoxplot(exoList[[3]])+ggtitle("rep3")+ylim(0,15))
dev.off()
...
## print(paramDistBoxplot(exoList[[3]],which.param = "beta2")+ggtitle("rep3")+ylim(0,45))
dev.off()

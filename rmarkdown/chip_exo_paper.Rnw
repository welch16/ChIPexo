...
\cite{exoillumina}, we divided the regions considered by the QC
pipeline into two sets by reviewing if they overlap any of the
sample's peaks and then calculate the Kolmogorov-Smirnov test.

...
designed to asses the quality levels of a ChIP-exo sample.
...
replicate, we sampled \Sexpr{strata} regions whenever possible (in
...
In order to asses the strand imbalance we created the following

...
\begin{abstractbox}
  ...
  ChIP-exo data is quite different from that of ChIP-Seq data. However,
...
replicate, we sampled \Sexpr{nstrata} regions whenever possible (in
...
\begin{figure}[h!]
  ...
  \caption{Using the mouse-FoxA1 experiment from \cite{exoillumina}:
    A) Hexbin plots of $\mbox{ARC}$ against $\mbox{URCR}$, B) Boxplots
    of the $\mbox{local-NSC}$ stratified by nr. of reads mapped to
    only one position in the regions for the ChIP-exo experiments
    divided by condition and biological replicate and C) Boxplots of
    the a set of ERR336942 regions, stratified as in (B) with the
    $\mbox{local-NSC}$ calculated with the different biological
    replicates.}
  \label{fig:enrich}
\end{figure}
...
\Sexpr{de} fragments into into the ones that overlap with any of the
sample's peaks. The p.value for ERR36935 is \Sexpr{pval[[1]]}, for
ERR336942 is \Sexpr{pval[[2]]} and for ERR336956 is \Sexpr{pval[[3]]}.
...
\begin{figure}[h!]
  ...
  \caption{Strand imbalance QC plots for the same data as in figure \ref{fig:enrich}.}
  \label{fig:strand}
\end{figure}

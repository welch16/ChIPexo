...
low $\mbox{ARC}$ values and $\mbox{URCR}$ values that can vary across
...
\Sexpr{nstrata} regions that whenever possible (in the opposite case,
...
%% perhaps do two other slots about how this works... a good experiment vs a bad experiment
%% of the same region
...
behavior as lower depth regions are being filtered out. while B)
...
asses this, we divided the ChIP-exo regions into two sets, the ones
that overlap with a known set of peaks and the opposite and performed
$\chi^2$ tests.

\begin{figure}[h!]
  \centering  
  \includegraphics[width = .8\textwidth]{../figs/fsr/draft_fsr_plot.pdf}
  \includegraphics[width = .8\textwidth]{../figs/fsr/draft_label_prop_plot.pdf}
  \caption{ABCD}
  \label{fig:strand}
\end{figure}
...
% And for
% the CexoR algorithm, we used its R implementation version 1.8.0 which
% is available from
...
%% not sure if here or before it would be nice to add an example of how this 
%% method works. probably here

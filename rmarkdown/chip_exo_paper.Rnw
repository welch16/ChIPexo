...
% ChIP-exo data for CTCF factir from human sample Rhee and Pugh
% (\cite{exo1}) as well. 
...
\begin{figure}[h]
  ...
  \caption{ A) ChIP signal in ChIP-exo was linearly related to that in
    ...
    empty. B) Density of the proportion of the ratio of forward strand
    reads in a ChIP-exo and ChIP-Seq SET data.}
  \label{fig:comp}
\end{figure}
...
\begin{figure}[h]
  \centering
  \includegraphics[width = .4\textwidth]{../figs/for_paper/sensitivity_exo_olda_data.pdf}
  \includegraphics[width = .4\textwidth]{../figs/for_paper/resolution_by_dataset_old_data.pdf}

...
In ChIP-exo experiments and input samples is often unavailable,
therefore to determine the quality of a ChIP-exo experiment is
necesary to address the balance between the enrichment and the library
complexity of the experiment. To diagnose this balance we considered
...
\begin{figure}[h!]
  ...
  \caption{A) For all $\sigma^{70}$ ChIP-exo experiments we are
    comparing the ARC against the URCR.}
  \label{fig:enrich}
\end{figure}
...
located in the other site of the binding site. 
...
\begin{figure}[h]
  ...
  % \includegraphics[width = .9\textwidth ,page = 3]{../figs/for_paper/MA_plot_fwd_bwd_by_seq.pdf}
...
% \subsection*{Strand cross-correlation for ChIP-exo data}

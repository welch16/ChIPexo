...
% Reffers to... First, DNA libraries... I think that these might be
...
events increases. When the average disntace is greater than 200 bp,
...
shorter than 200 bp, hence the read doesn't contribute to more than
...
ChIP-Seq to assume more realistic situation of a fixed cost). For
each sampled dataset we applied out lower-to-higher resolution
...
ChIP-exo datasets we called peaks by using the GC-content and
...
reads is used for all of ChIP-exo, SE ChIP-Seq and PE ChIP-Seq. It is
remarkable that even when the number of candidate peaks or the number
of predicted events is lower for ChIP-exo, it outperforms both PE and
SE ChIP-Seq in number of identified targets and resolution.
...
ChIP-exo's lower library complexity. Additionally, it is worth noting
that for PE ChIP-Seq we sampled both ends of the fragment, hence for
each sequencing depth we are sampling the half amount of pairs for PE
...
annotation in a are within \Sexpr{ext} bp from an estimated binding event.
...
gold-standard annotations within \Sexpr{ext} bp from an estimated binding

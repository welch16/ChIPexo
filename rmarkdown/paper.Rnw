...
\begin{abstractbox}
  ...
  \begin{abstract}
    ...
    present a quality control pipeline that analyzes a ChIP-exo and
    ChIP-nexus experiment's strand imbalance, enrichment and library
...
% \subsection*{Evaluation of QC visualizations}

% Each boxplot in Figure \ref{fig:enrich_all}A, was created by sampling
% \Sexpr{Msamp} of the regions generated and fitting the model:

% \begin{align*}
%   \mbox{depth} = \beta \times \text{(\# unique
%       positions)}
% \end{align*}

% and repeating the process \Sexpr{Ntimes} times. 

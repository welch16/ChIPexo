...
% that both
% local maxima are hard to differentiate in the SCC curves for the
% $\sig$ ChIP-exo datasets used to calculate the NSC values in Table
% \ref{tab:qc}.

% It is calculated as the correlation between both strand
...
% To verify this statement, we
% extracted the sequences around high confidence binding events and look
% for the FoxA1 motif using FIMO \cite{fimo}. Figure \ref{fig:enrich}B
% shows the number of candidate regions, which shows that the first
% replicate is being allocated into more enriched regions than the other
% ones. Figure \ref{fig:enrich}C shows that for the first and third
% replicates, the FoxA1 motif is being detected in roughly the same
% proportion of sites, and finally in \ref{fig:enrich}D we observe that
% the first and third replicate can detect the FoxA1 motif with the same
% significance, while the second replicate does not.
...
% \begin{enumerate}
% \item \st{Available data, the motivation of this part is two fold:}
%  \begin{itemize}
%  \item \st{we want to list all data sets we obtained}
%  \item \st{we want to emphasize that for the sig70 datasets we have a
%    unique design that allows us to compare chip exo with SE and PE
%    chipseq , the later one for the first time}
%  \end{itemize}
% \item \st{comparison with chip seq} %? with and without dpeak ?}
% \item \st{only in methods. list the align algorithm used}
% \item \st{chip seq qc metrics}
% \item \st{chip exo qc pipeline, description and justification of the steps}
% \begin{itemize}
% \item \st{need to justify the sampling step. for this we may use the depth
...
% \item \st{case study with FoxA1, analysis of enrichment vs library
...
%   the plots.}
% \item \st{analysis of why the pipeline works. in methods need to specify
...
% For each sampled dataset we applied our lower-to-higher resolution
% pipeline by calling peaks with MOSAiCS \cite{mosaics} and then
% deconvolving the binding events by using dPeak \cite{dpeak}. For the
% ChIP-exo experiments we called peaks by using the GC-content and
% mappability models with MOSAiCS, since it's background is usually
% composed of scattered reads across the genome; and for the ChIP-Seq
% datasets we used their respective Input samples.
...
% When the average distance is greater than 200 bp,
...
non-overlaping bins of length \Sexpr{bin_size}, and for each bin we
counted how many ChIP-exo and SE ChIP-Seq extended reads overlapped
each bin. The reads were extended by \Sexpr{frag_len} bp to their
$3\prime$ directions.
...
\begin{figure}[h!]
\centering
\includegraphics[width = .8 \textwidth]{/p/keles/ChIPexo/volume3/ChIPexo/figs/for_paper/scc_ctcf2.pdf}
...
\begin{figure}[h!]
  ...
  \newline
  \includegraphics[width = .23 \textwidth, page =
  1]{../figs/for_paper/FoxA1_barplots.pdf}
  \includegraphics[width = .23 \textwidth, page =
  3]{../figs/for_paper/FoxA1_barplots.pdf}
  \includegraphics[width =
  .4\textwidth]{../figs/for_paper/FoxA1_profiles_around_motif.pdf}
  \includegraphics[width = .3\textwidth,page =
  2]{../figs/for_paper/FoxA1_matched_motif_sequence.pdf}
  \includegraphics[width = .3\textwidth,page =
  3]{../figs/for_paper/FoxA1_matched_motif_sequence.pdf}
  \includegraphics[width = .3\textwidth,page =
  1]{../figs/for_paper/FoxA1_matched_motif_sequence.pdf}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width = .9  \textwidth,page = 1]{../figs/for_paper/QC_pipeline_eval_boxplot.pdf}
\newline
  \includegraphics[width = .9  \textwidth,page = 2]{../figs/for_paper/QC_pipeline_eval_boxplot.pdf}
\newline
  \includegraphics[width = .45  \textwidth,page = 1]{../figs/for_paper/K562_TBP_sample.pdf}
  \includegraphics[width = .45  \textwidth,page = 2]{../figs/for_paper/K562_TBP_sample.pdf}
...
\end{figure}
  ...
  % \caption{A) Comparison of the contribution of the number of unique
  %   positions to depth among all ChIP-exo and ChIP-nexus datasets
  %   separated by organism. Comparison of the contributions of number
  %   of unique positions (B) and width (C) to depth for sub-samples of
  %   20M to 50M reads of TBP factor in human K562 cell lines ChIP-exo
  %   and ChIP-nexus experiments.}

\newpage

\begin{figure}[h!]
  \centering
  \includegraphics[width = .45\textwidth,page = 1]{../figs/for_paper/methods_comp_FDR5_topM400.pdf}
  \includegraphics[width = .45\textwidth,page = 2]{../figs/for_paper/methods_comp_FDR5_topM400.pdf}
   \includegraphics[width = .45\textwidth,page = 1]{../figs/for_paper/sigma_delta_old_densities.pdf}
   \includegraphics[width = .45\textwidth,page = 2]{../figs/for_paper/sigma_delta_old_densities.pdf}   

...
(courtesy of Professor Robert Landick's lab).
...
\subsubsection{Analysis of ChIP-Nexus data with the QC pipeline}

ChIP-nexus developed by He et al., 2015 \cite{chipnexus} is a
modification of the ChIP-exo protocol, where both sequencing adaptors
are ligated at the $3\prime$ end of the ChIP fragments. Then,
exonuclease digestion and DNA self-circularization with circLigase,
and restriction enzyme cutting between the two adaptors to create the
final library that is amplified. 
...
\Sexpr{ext} bp from an estimated binding event.
